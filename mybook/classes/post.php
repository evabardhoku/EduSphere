...
class Post
{
        ...
        if (!empty($data['post']) || !empty($files['file']['name']) || isset($data['is_profile_image']) || isset($data['is_cover_image'])) {
            ...
            if (isset($data['is_profile_image']) || isset($data['is_cover_image'])) {
                ...
                if (!empty($files['file']['name'])) {
                    ...
                    if ($files['file']['error'] == UPLOAD_ERR_OK && in_array($files['file']['type'], $allowed)) {
                        ...
                        $this->error .= "The selected image is not a valid type. Only JPEGs are allowed!<br>";
                    }
                }
            }
            ...
            if ($this->error == "") {
                ...
                $query = "INSERT INTO posts (userid, postid, post, image, has_image, is_profile_image, is_cover_image, parent, tags, likes) 
                      VALUES ('$userid', '$postid', '$post', '$myimage', '$has_image', '$is_profile_image', '$is_cover_image', '$parent', '$tags', 0)";
                $DB->save($query);
    ...
    public function like_post($id, $type, $mybook_userid)
    {
        ...
        if (is_array($result)) {
            $likes = json_decode($result[0]['likes'], true);
            if ($likes === null) {
                $likes = []; // Initialize as an empty array if json_decode fails
            }
    ...
    //funksionet per postet e grupeve
    public function get_one_group_post($postid)
    {
        // Ensure the post ID is numeric
        if (!is_numeric($postid)) {
            return false;
        }

        // Query to fetch the post from the group_posts table using 'id' as the column name
        ...
        // If the result is found, return the first row, else return false
        if ($result) {
            return $result[0];
        } else {
            return false;
    ...
    public function get_group_posts($userid, $groupid = null)
    {
        $page_number = isset($_GET['page']) ? (int)$_GET['page'] : 1;
        $page_number = ($page_number < 1) ? 1 : $page_number;
        ...
        $query = "SELECT * FROM posts WHERE parent = 0 AND userid = '$userid'";
    ...
    public function get_group_comments($postid, $groupid = null)
    {
        $page_number = isset($_GET['page']) ? (int)$_GET['page'] : 1;
        $page_number = ($page_number < 1) ? 1 : $page_number;
        ...
        $query = "SELECT * FROM posts WHERE parent = '$postid'";
